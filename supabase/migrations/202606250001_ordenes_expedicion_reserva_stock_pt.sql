-- Flujo transaccional de órdenes de salida sobre stock PT.
-- Reserva en stock_pt.cantidad_comprometida, despacho descuenta stock físico y cancelación libera o revierte.

alter table public.stock_pt
  add column if not exists cantidad_comprometida numeric(14,3) not null default 0;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'stock_pt_cantidad_comprometida_non_negative'
  ) then
    alter table public.stock_pt
      add constraint stock_pt_cantidad_comprometida_non_negative
      check (cantidad_comprometida >= 0);
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'stock_pt_comprometida_lte_total'
  ) then
    alter table public.stock_pt
      add constraint stock_pt_comprometida_lte_total
      check (cantidad_comprometida <= cantidad_total);
  end if;
end;
$$;
...
begin
  ...
  if (v_stock_pt.cantidad_total - v_stock_pt.cantidad_comprometida) < v_cantidad_kg then
...
begin
  ...
  if not found then raise exception 'La orden de expedición no existe.'; end if;
  ...
  if not found then raise exception 'El stock PT no existe.'; end if;

  if v_delta > 0 and (v_stock_pt.cantidad_total - v_stock_pt.cantidad_comprometida) < v_delta then
    raise exception 'No hay saldo suficiente en el lote de PT.';
  end if;

  update public.stock_pt
  set cantidad_comprometida = greatest(0, cantidad_comprometida + v_delta),
      updated_at = now()
  where id = v_stock_pt.id;
...
begin
  select * into v_orden from public.ordenes_expedicion where id = p_orden_id for update;
  if not found then raise exception 'La orden de expedición no existe.'; end if;
  if v_orden.estado <> 'pendiente' then raise exception 'La orden ya fue despachada o cancelada.'; end if;
...
begin
  select * into v_orden from public.ordenes_expedicion where id = p_orden_id for update;
  if not found then raise exception 'La orden de expedición no existe.'; end if;
  if v_orden.estado = 'cancelada' then raise exception 'La orden ya fue cancelada.'; end if;
  ...
  if v_orden.estado = 'pendiente' then
    update public.stock_pt
    set cantidad_comprometida = greatest(0, cantidad_comprometida - v_orden.cantidad_kg),
        updated_at = now()
    where id = v_stock_pt.id;
  else
    update public.stock_pt
  ...
  insert into public.stock_pt_movimientos (
    ...
    case when v_orden.estado = 'pendiente' then 'Liberación de reserva por cancelación' else 'Reverso de despacho de orden cancelada' end,
