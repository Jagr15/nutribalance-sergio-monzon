...
begin
  ...
  v_costo_unitario_cxc := coalesce(nullif(v_stock_pt.costo_unitario_estimado, 0), v_costo_unitario_fallback, 0);
  v_valor_total := round(v_orden.cantidad_kg * v_costo_unitario_cxc, 6);
  ...
  if v_orden.cliente_id is not null and v_valor_total > 0 then
    ...
    insert into public.flujo_caja_movimientos (
      ...
      jsonb_build_object(
        ...
        'costo_unitario_cxc', v_costo_unitario_cxc,
        'costo_unitario_origen', case
          when coalesce(v_stock_pt.costo_unitario_estimado, 0) > 0 then 'stock_pt'
          when v_costo_unitario_fallback > 0 then 'orden_produccion_fallback'
          else 'none'
        end,
