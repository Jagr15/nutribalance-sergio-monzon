...
declare
  ...
  v_costo_total_consumos numeric;
...
begin
  ...
  where s.deleted_at is null
    ...
    and s.tipo_uso = 'PRODUCTO_TERMINADO'
    and (
      s.id::text = p_destino_silo
      or s.legacy_uid = p_destino_silo
      or lower(btrim(s.nombre)) = lower(btrim(p_destino_silo))
    )
  limit 1;

  if v_silo.id is null then
  ...
  select coalesce(sum(coalesce(ocl.costo_total, 0)), 0)
  into v_costo_total_consumos
  from public.orden_consumo_lotes ocl
  where ocl.orden_id = v_orden.id;

  if v_costo_total_consumos > 0 then
    update public.ordenes_produccion op
    set
      costo_total_insumos = v_costo_total_consumos,
      updated_at = now()
    where op.id = v_orden.id;

    v_orden.costo_total_insumos := v_costo_total_consumos;
  end if;
  ...
  for v_item in
    ...
    left join public.insumos ins_legacy
      ...
      and ins_legacy.esta_activa = true
    left join public.insumos ins_nombre
      ...
      and ins_nombre.esta_activa = true
    where ocl.orden_id = v_orden.id
    order by ocl.id asc
  loop
