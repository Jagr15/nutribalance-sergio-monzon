...
begin
  ...
  for v_item in
    select
      ...
      ocl.nombre_insumo,
      ocl.cantidad_usada,
      coalesce(ocl.lote_id, sl_legacy.id, sl_nombre.id) as lote_id_resuelto
    from public.orden_consumo_lotes ocl
  ...
  loop
    ...
    for v_lote in
      ...
      where sl.deleted_at is null
        and (
          sl.insumo_id = (select id from public.insumos where legacy_uid = v_item.id_insumo_legacy and deleted_at is null limit 1)
          or i.legacy_uid = v_item.id_insumo_legacy
          or sl.legacy_uid = v_item.id_insumo_legacy
          or sl.lote = v_item.id_lote_legacy
        )
    ...
    loop
      ...
      set cantidad_comprometida = greatest(0, coalesce(sl.cantidad_comprometida, 0) - v_consumido_lote)
  ...
  insert into public.trazabilidad_eventos (
    ...
    v_orden.id,
    'FINALIZACION_OP',
    format('Finalización de OP %s', coalesce(v_orden.legacy_uid, v_orden.lote)),
    jsonb_build_object(
      'orden_id', v_orden.id,
      'orden_legacy_uid', v_orden.legacy_uid,
      'cantidad_real', p_cantidad_real,
