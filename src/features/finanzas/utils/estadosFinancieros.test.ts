  ...
  it('construye estado de resultados y balance desde movimientos reales', () => {
    const result = buildEstadosFinancieros({
      ...
      movimientos: [
        ...
        { uid: '2', fecha: '2026-06-11T00:00:00Z', tipo: 'EGRESO', descripcion: 'Compra MP', monto: 2000, origen_operativo: 'COMPRA_MP', estado: 'CONFIRMADO' },
      ],
      kpis: {
        saldo_actual: 1000,
        ingresos_mes: 5000,
    ...
    expect(result.libros.auxiliarIngresos[0]).toMatchObject({ label: 'VENTA_PT', amount: 5000 });
  });
});
