...
export const KpiGrid = ({ kpis }: { kpis: FinanzasKPIs }) => (
  <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
    <Card><p className="text-xs text-gray-400">Saldo actual</p><p className="text-3xl font-black mt-2 text-cyan-300">{ars(kpis.saldo_actual)}</p></Card>
    <Card><p className="text-xs text-gray-400">Ingresos mes</p><p className="text-3xl font-black mt-2 text-emerald-300">{ars(kpis.ingresos_mes)}</p></Card>
    <Card><p className="text-xs text-gray-400">Egresos mes</p><p className="text-3xl font-black mt-2 text-red-300">{ars(kpis.egresos_mes)}</p></Card>
    ...
    <Card><p className="text-xs text-gray-400">Costo producción</p><p className="text-2xl font-black mt-2 text-amber-300">{ars(kpis.costo_produccion)}</p></Card>
    <Card><p className="text-xs text-gray-400">Valorización inventario</p><p className="text-2xl font-black mt-2 text-fuchsia-300">{ars(kpis.valorizacion_inventario)}</p></Card>
